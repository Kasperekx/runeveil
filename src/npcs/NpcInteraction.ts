...
import type { ItemInstance } from "../items/catalog";
import type { GameNetwork } from "../network/GameNetwork";
import type { DialogueQuestAction, DialogueWindow } from "../ui/DialogueWindow";
import type { GameToast } from "../ui/GameToast";
import { getNpc } from "./catalog";
import type { NpcSystem } from "./NpcSystem";

const TOO_FAR_MESSAGE = "Podejdź bliżej, by porozmawiać.";

/**
 * Click an NPC to select it and open its dialogue; click elsewhere to end the
 * conversation. Trade actions go through GameNetwork (server-authoritative).
 */
export class NpcInteraction {
  ...
  constructor(
    ...
    /** Allows other world props to share the hand cursor and click priority. */
  ...
  start(): void {
    this.dialogue.bindTrade(this.inventory, {
  ...
  private onPointerDown = (event: PointerEvent): void => {
    ...
    if (!hit) {
      // Cooking fires and future interactive props claim their own click.
    ...
    // Claim the click so PlayerCombat doesn't also try to attack/deselect.
  ...
  /** Closes the window if the player wanders off mid-conversation. */
  private update = (): void => {
    if (!this.openNpcId || !this.dialogue.isOpen) return;
