...
import type {
  CampfirePlacedEvent,
  CampfireRemovedEvent,
  CampfiresStateEvent,
  ChatMessageEvent,
  CombatTextEvent,
  EquipmentBrokenEvent,
  EquipmentRepairedEvent,
  FoodBuffStateEvent,
  ItemSnapshot,
  ItemUsedEvent,
  LevelUpEvent,
  LootDroppedEvent,
  MapTransitionEvent,
  MapTransitionRejectedEvent,
  MiningNodeDepletedEvent,
  MiningNodeRespawnedEvent,
  MiningNodeStateEvent,
  NetworkAnimalSnapshot,
  NetworkPickupSnapshot,
  NetworkPlayerSnapshot,
  NetworkRemotePlayerSnapshot,
  NoticeEvent,
  OreMinedEvent,
  PlayerDeathEvent,
  PlayerRespawnedEvent,
  ProfessionCraftedEvent,
  ProfessionSnapshot,
  QuestAcceptedEvent,
  QuestClaimedEvent,
  QuestReadyEvent,
...
export class GameNetwork {
  ...
  onResourceChange:
    | ((state: { kind: string; resource: number; maxResource: number }) => void)
    | null = null;
  ...
  private bindMiningMessages(room: Room): void {
    ...
    room.onMessage("miningNodeRespawned", (event: MiningNodeRespawnedEvent) => {
      if (this.miningNodesState) {
        this.miningNodesState = {
  ...
  private readOwnSnapshot(): NetworkPlayerSnapshot | null {
    ...
    return {
      ...
      bonusStrength: mine.bonusStrength ?? 0,
      bonusAgility: mine.bonusAgility ?? 0,
      bonusStamina: mine.bonusStamina ?? 0,
      bonusIntellect: mine.bonusIntellect ?? 0,
      bonusSpirit: mine.bonusSpirit ?? 0,
